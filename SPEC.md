...
### 확정된 UX 상세 (2026-07-02, 목업 승인)
- **롤 = 물리적 코일 덩어리 1개, M(미터) 단위로 잔량 추적.** 색상 1개는 보통 롤 1개(단일)이고, 실제로 덩어리가 여러 개인 색상만 롤이 2~3개.
- **코일관리 화면**: 색상별 카드 → 펼치면 그 색상의 롤이 한 줄씩. 단일이면 라벨 없이 한 줄, 덩어리 2개 이상일 때만 A·B·C 자동 라벨. 카드 헤더에 색상 총합 M + 롤 개수(코일에서 계산).
- **입고/롤추가 통합**: 입고 1건 = 새 롤(coils 문서) 1개 생성. 기존 롤이 남아있어도 새 입고는 **별도 덩어리**로 쌓임(물리 현실 반영). 실사 등록도 같은 흐름, `source: 입고|실사`로만 구분.
- **출고**: 색상 선택 → 덩어리(롤) 선택(단일이면 자동) → 출고 M 입력 시 **실시간 계산**(그 롤의 `현재재고 → 출고 → 남는재고` + 색상 총합). 선택 롤에서 차감, 초과 시 경고 또는 다음 오래된 롤로 이어서 차감. 0M 롤은 "사용완료" 처리.

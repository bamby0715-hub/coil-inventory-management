...
/* =========================================================================
   입고(inbound) — 건별 개별 문서 컬렉션 (B 구조)
   - 컬렉션: inbound/{id}   (id = 기존 레코드의 uid)
   - 한 문서에 모아두던 방식(hnmtCoilSystem/sharedState.snapshot.inbound)을
     대체한다. 이제 입고 1건 = 문서 1건.
   - 화면/재고 로직은 그대로: useInboundStore() 가 기존 useStore("inbound")와
     똑같이 [inbound, setInbound] 를 돌려준다. setInbound 호출부는 수정 불필요.
...
const inboundCol = (fs, db) => fs.collection(db, "inbound");
const inboundDocRef = (fs, db, id) => fs.doc(db, "inbound", id);

// ---------- 실시간 구독 ----------
// 정렬은 쿼리에서 하지 않는다(필드 누락 문서가 빠질 수 있으므로). 화면 쪽 기존
// 클라이언트 정렬(inbound_date 등)이 그대로 처리한다.
export function subscribeInbound(onData, onError) {
  let unsub = () => {};
  getDataRuntime().then(({ db, fsModule }) => {
    unsub = fsModule.onSnapshot(
      inboundCol(fsModule, db),
...
// ---------- 단건 쓰기 ----------
export async function writeInbound(id, record) {
  const { db, fsModule } = await getDataRuntime();
  // 레코드 전체를 그대로 저장(필드 비종속). updatedAt 만 서버 시각으로 덧붙인다.
  await fsModule.setDoc(inboundDocRef(fsModule, db, id), {
    ...record,
    id,
...
export async function deleteInbound(id) {
  const { db, fsModule } = await getDataRuntime();
  await fsModule.deleteDoc(inboundDocRef(fsModule, db, id));
}
...
/* -------------------------------------------------------------------------
   useInboundStore — 기존 useStore("inbound") 자리에 그대로 끼우는 어댑터.
   반환: [inbound, setInbound]  (setInbound 는 값 또는 (prev)=>next 둘 다 지원)
   - 마운트 시 컬렉션을 구독해 항상 최신 상태 유지(다른 사람 변경도 실시간 반영).
   - setInbound 호출 시 즉시 화면 반영(낙관적) + 바뀐 건만 Firestore 에 기록.
   - 구독으로 들어온 갱신은 다시 쓰지 않는다(되먹임 방지).
   ------------------------------------------------------------------------- */
export function useInboundStore() {
  const [inbound, setLocal] = useState([]);
  const ref = useRef([]);
  ref.current = inbound;

  useEffect(() => {
    const unsub = subscribeInbound(
      (list) => { ref.current = list; setLocal(list); },
      (e) => { console.error("입고 구독 실패:", e?.message || e); },
    );
    return unsub;
  }, []);

  const setInbound = useCallback((updater) => {
    const prev = ref.current;
    const next = typeof updater === "function" ? updater(prev) : updater;
    ref.current = next;
    setLocal(next); // 즉시 화면 반영
    syncInboundDiff(prev, next).catch((e) => {
      console.error("입고 동기화 실패:", e?.message || e);
    });
  }, []);

  return [inbound, setInbound];
}
